...
export const checkUserAuthentication = () => {
  // Check new token-based auth first
...
export const getUserData = () => {
  ...
  if (user) {
    try {
      ...
      // Convert role to type for backward compatibility
      return {
        ...userData,
...
export const getAllSpecialists = () => {
  // Bu funksiya endi backend'dan ma'lumot olish kerak
  // Hozircha bo'sh array qaytaramiz
  console.log("getAllSpecialists: This should fetch from backend API");
  
  const users = JSON.parse(localStorage.getItem("fixoo_users") || "[]")
  const specialists = users.filter((user: any) => user.type === "specialist" || user.role === "specialist")
...
// Logout user
export const logout = () => {
  // Clear both old and new auth data
  localStorage.removeItem("fixoo_current_user")
