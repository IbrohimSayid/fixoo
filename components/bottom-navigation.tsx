...
import { PlusCircle, ClipboardList, Settings } from "lucide-react"
      ...
      <div className="flex justify-around items-center h-16">
        <Link
          ...
          className={`flex flex-col items-center justify-center w-full h-full ${
            isActive("/new-orders") ? "text-white" : "text-white/70"
          }`}
        >
          <PlusCircle className="h-6 w-6" />
          <span className="text-xs mt-1">{getTranslation("newOrders", language)}</span>
        </Link>
